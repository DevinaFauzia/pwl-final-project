<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-2xl text-gray-800 leading-tight">Stock Opname</h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100">
                <div class="p-8">
                    ...
                    <form action="{{ route('warehouse.stock.opname.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-6">
                            <label for="actual_stock" class="block text-sm font-medium text-gray-700 mb-2">Stok Aktual</label>
                            <input type="number" name="actual_stock" id="actual_stock" value="{{ old('actual_stock') }}" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required min="0">
                        </div>

                        <div class="mb-6">
                            <label for="date" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Opname</label>
                        ...
                        <div class="flex items-center justify-end space-x-4 mt-8">
                            ...
                            <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-6 rounded-lg shadow-md transition duration-300 transform hover:-translate-y-1">Simpan Opname</button>
                        </div>
                    </form>
                </div>
            </div>
